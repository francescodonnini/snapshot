...
out:
    rcu_read_unlock();
    if (!err) pr_info("registry_add_range: successfully added (%lu, %lu)", range->start, range->end_excl);
    if (err == -EEXIST) pr_info("registry_add_range: (%lu, %lu) already exists", range->start, range->end_excl);
    return err;
}
